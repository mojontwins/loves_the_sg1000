...
Dim As Integer fIn, fIn2, x, y, pattern, row, xx, yy, plane, x0, bank, fine, mbtn
Dim As uByte d 
Dim As uByte planes (1, 7)
Dim As Integer colours (15) = { _
...
Dim As String p2d
Dim As Integer pn, opn, bn, obn, c1, c2

If Command (1) = "" Then ? "$ chrview pats.bin cols.bin [save]": System

If Command (3) = "save" Then
...
fIn2 = FreeFile
Open Command (2) For Binary As #fIn2
If Lof (fIn2) = 0 Then
	Close #fIn2
	Kill Command (2)
	System
...
For bank = 0 To 1
	...
	For pattern = 0 To 255
		...
		For row = 0 To 7
			Get #fIn, , planes (0, row)
			Get #fIn2, , planes (1, row)
		Next row

		For yy = 0 To 7
			c2 = planes (1, yy) Shr 4
			c1 = planes (1, yy) And 15

			For xx = 0 To 7


				d = c1
				If planes (0, yy) And (1 Shl (7 - xx)) Then d = c2
