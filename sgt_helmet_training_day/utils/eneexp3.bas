' ene exporter v0.3 [20181023]
' generates minimal contents for usage in small ROMs.

sub usage
	Print "Usage:"
	Print 
	Print "$ eneexp3 enems.ene out[.h] prefix [yadjust] [nohotspots] [gencounter] [genallcounters] [bin] [cwithheader]"
	Print
...
If cWithHeader Then
	fOut = FreeFile
	Open Command (2) & ".c" For Output As #fOut
	fOutHeader = FreeFile
...
If Not binMode Then
	If cWithHeader Then
		Print #fOut, "// " & Command (2) & ".c"
		Print #fOut, "// Generated by eneexp3.exe v0.3 [20181023]"
		Print #fOut, "// Copyleft 2016, 2018 The Mojon Twins"
		Print #fOut, ""
		Print #fOutHeader, "// " & Command (2) & ".h"
		Print #fOutHeader, "// Generated by eneexp3.exe v0.3 [20181023]"
		Print #fOutHeader, "// Copyleft 2016, 2018 The Mojon Twins"
		Print #fOutHeader, ""
	Else
	...
	If cWithHeader Then
		Print #fOutHeader, "extern const unsigned char enems_" & prefix & " [];"
	End If
Else
...
For i = 1 To 255
	If enemTypeCounters (i) Or (i < 32 And genAllCounters) Then
		If cWithHeader Then
			Print #fOutHeader, "#define MAX_ENEMS_TYPE_" & i & "_" & prefix & " " & enemTypeCounters (i)
		Else
...
If genCounter Then
	If cWithHeader Then
		Print #fOutHeader, "#define KILLABLE_ENEMS_" & prefix & " " & (enemTypeCounters (1) + enemTypeCounters (2) + enemTypeCounters (3) + enemTypeCounters (&H41) + enemTypeCounters (&H42) +enemTypeCounters (&H43) + enemTypeCounters (&H81) + enemTypeCounters (&H82) + enemTypeCounters (&H83) + enemTypeCounters (6) + enemTypeCounters (9) + enemTypeCounters (11) + enemTypeCounters (20) )
		Print #fOutHeader, ""
	Else
...
If Not noHotspots Then
	If Not binMode Then
		...
		If cWithHeader Then
			Print #fOutHeader, "extern const unsigned char hotspots_" & prefix & " [];"
		End If
	End If
	hl = 0
	For i = 1 To mapPants
	...
	For i = 0 To 255
		If typeCounters (i) <> 0 Or i = 1 then
			If cWithHeader Then
				Print #fOutHeader, "#define MAX_HOTSPOTS_TYPE_" & i & "_" & prefix & "	" & typeCounters (i)
			Else
	...
	If binMode Then 
		Print #fOut, ""
		If cWithHeader Then Print #fOutHeader, ""
	End If
