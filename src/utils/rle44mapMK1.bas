' Map exporter (RLE44) v0.4 [20181024]
...
Sub usage
	Print "Usage:"
	Print 
	Print "$ rle44mapMK1.exe in.map out.h w h tlock prefix [offset] [scrsizes] [nodecos] [bin] [cwithheader]"
	Print
	Print "cwithheader requires that 'out' doesn't have an extension and generates .h and .c"
	Print "cwithheader WILL DISABLE BINMODE!"
End Sub
...
Dim As Integer x, y, xx, yy, nPant, mapW, mapH, i, j, dat, f, fBin, mapsize, decosize, scrsize, founddecos, fOutHeader
Dim As Integer printscrsizes, nodecos, offset, binMode, cWithHeader
...
cWithHeader = inCommand ("cwithheader")

If cWithHeader Then binMode = 0
...
If cwithheader Then
	f = FreeFile
	Open Command (2) & ".c" For Output As #f
	fOutHeader = FreeFile
	Open Command (2) & ".h" For Output As #fOutHeader
	Print #f, "// rle44mapMK1(TM) map " & Command (2) & ".c"

	Print #fOutHeader, "// rle44mapMK1(TM) map " & Command (2) & ".h"
	Print #fOutHeader, "// Generated by rle44mapMK1.exe v0.4 [20181024]"
	Print #fOutHeader, "// Copyleft 2015, 2017, 2018 The Mojon Twins"
	Print #fOutHeader, ""
...
Print #f, "// Generated by rle44mapMK1.exe v0.4 [20181024]"
...
If Not binMode Then
	...
	If cwithheader Then
		Print #fOutHeader, "extern const unsigned char * const map_" & prefix & " [];"
		Print #fOutHeader, ""
	End If		
...
If locksI Then
	...
	Else
		...
		print #f, ""
		If cWithHeader Then
			Print #fOutHeader, "#define N_BOLTS_" & Ucase (prefix) & " " & (locksI \ 2)
		Else
			Print #f, "#define N_BOLTS_" & Ucase (prefix) & " " & (locksI \ 2)
			print #f, ""
		End If		
		...
		If cWithHeader Then
			Print #fOutHeader, "extern const unsigned char map_" & prefix & "_locks [];"
			Print #fOutHeader, ""
		End If
		Print #f, "	";
		For i = 0 To locksI - 1
