...
Sub doCutOut (img As Any Ptr, x0 As Integer, y0 As Integer, x1 As Integer, y1 As Integer)
	Select Case platform
		Case PLATFORM_CPC
			cpcDoNametable img, x0, y0, x1, y1
		Case PLATFORM_ZX
...
Sub writeCutoutText (label As String)
	...
	Print #fMapFile, "// cutout data (" & cutoutX0 & ", " & cutoutY0 & ") - (" & cutOutX1 & ", " & cutOutY1 & ")."
	If platform = PLATFORM_ZX Then
		Print #fMapFile, "const unsigned char " & label & "_patterns [] = {"
		If fMapFile <> fHeaderFile Then Print #fHeaderFile, "extern const unsigned char " & label & "_patterns [];"
	Else
		Print #fMapFile, "const unsigned char " & label & " [] = {"
		If fMapFile <> fHeaderFile Then Print #fHeaderFile, "extern const unsigned char " & label & " [];"
	End If
	...
	If platform = PLATFORM_ZX Then
		bytes2 = bytes: bytes = 0
		' Attributes
		Print #fMapFile, "const unsigned char " & label & "_attributes [] = {"
		If fMapFile <> fHeaderFile Then Print #fHeaderFile, "extern const unsigned char " & label & "_attributes [];"
		For y = cutOutY0 To cutOutY1
			For x = cutOutX0 To cutOutX1
				If bytes Mod 8 = 0 Then Print #fMapFile, "    ";
				d = nametable (768+y*32+x): Print #fMapFile, "0x" & lCase (Hex (d, 2));
				If i < sizeT Then Print #fMapFile, ", ";
				If bytes Mod 8 = 7 Then Print #fMapFile, ""
				bytes = bytes + 1
		Next x, y
...
Sub zxDoScripted (scriptFile As String)
	...
	Dim As Integer xc0, yc0, xc1, yc1, w, h, wMeta, hMeta, max, imgOn, palOn, i
	...
	While Not Eof (fIn)
		...
		Select Case Lcase (tokens (0))
			...
			Case "write"
				...
				If tokens (1) = "cutout" then writeCutoutText tokens (2)
			...
			Case "nametable"
				If tokens (1) = "cls" Then
					For i = 0 To 1535
						nametable (i) = 0
					Next i
				ElseIf tokens (1) = "rle" Then
					doRle
				Else
					doNametable img
				End If
			...
			Case "cutout"
				cutoutX0 = Val (tokens (1))
				cutoutY0 = Val (tokens (2))
				cutoutX1 = Val (tokens (3))
				cutoutY1 = Val (tokens (4))

				doCutOut img, cutoutX0, cutoutY0, cutoutX1, cutoutY1
