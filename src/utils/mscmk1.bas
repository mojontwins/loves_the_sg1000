...
Dim Shared As uInteger isSega

Sub usage
	Print "Usage:"
	Print
	Print "$ mscmk1.exe input.spt output.h maxpants [debug] [bin]"
End Sub
			...
			Else
				...
				Select Case Ucase (lP (0))
					...
					Case "MUSIC"
						...
						Else
							If isSega Then
								Print "WARNING - MUSIC doesn't work in sg1000 yet."
							End If
					...
					Case "SOUND"
						If isSega Then
							clausula = clausula + Chr (&HE0) + Chr (pValR (lP (1))) + Chr (pValR (lP (2)))
						Else
...
Print "MK1 v2.0 mscmk1 1.2 ~ ";
If Len (Command (3)) = 0 Then usage: End
debug = inCommand ("debug")
binmode = inCommand ("bin")
doubleWidth = inCommand ("doublewidth")
isSega = inCommand ("sg1000")
If binmode Then Print "[bin] ~ ";

If isSega Then
	fOut = FreeFile
	Open Left (Command (2), InStrRev (Command (2), ".")) & "h" For Output As #fOut
		Print #fOut, "// SG1000 MK1 v0.x"
...
Print #fOut, ""
If binmode Then
	Print #fOut, "// " & Command (2) & " - Scripts interpreter."
	Print #fOut, "// Script pools exported to " & Command (2) & ".bin"
Else
	Print #fOut, "// " & Command (2) & " - Script pools and scripts interpreter"
End If
Print #fOut, "// generated by mscmk1 1.2 by the Mojon Twins"
If doubleWidth Then
...
If isSega Then
	Print #fOut, "#include ""../lib/SGlib.h"""
	Print #fOut, "#include ""../lib/PSGlib.h"""
	Print #fOut, "#include ""../murcia.h"""
	Print #fOut, "#include ""../definitions.h"""
	Print #fOut, "#include ""../config.h"""
	Print #fOut, "#include ""../autodefs.h"""
	Print #fOut, "#include ""../ram/extern_globals.h"""
	Print #fOut, "#include ""../engine/extern_precalcs.h"""
	Print #fOut, "#include ""../utils/delay.h"""
	Print #fOut, "#include ""../engine/printer.h"""
	Print #fOut, "#include ""../engine/general.h"""
	Print #fOut, "#include ""../assets/msc_sfx.h"""
	Print #fOut, ""
End If
	...
	Select Case lP (0)
		...
		Case "END_OF_LEVEL"
			...
			If sDone Then
				...
				If binmode Then
					Print #fOut, "#define SCRIPT_POOL_" & scriptCount & "_OFFSET 0x" & Hex (fullBinOffset, 4)
					For i = 0 To binPt - 1
						d = myBin (i)
						Put #fBin, , d
					Next i
					fullBinOffset = fullBinOffset + binPt
				Else
...
If binmode Then
	...
	Print #fOut, "    // Now the correct bank is paged in and"
	Print #fOut, "    // gp_gen contains a pointer to the pool"
	Print #fOut, "    script = (unsigned char *) (gp_gen + script_pool [level] + (whichs << 1));"
	Print #fOut, "    rda = *script ++; rdb = *script;"
	Print #fOut, "    script_result = 0;"
	Print #fOut, "    if (!(rda | rdb)) return;"
	Print #fOut, "    script = (unsigned char *) (gp_gen + script_pool [level] + rda + (rdb << 8));"	
Else
	Print #fOut, "    gp_gen = (unsigned char *) script_pool [level] + (whichs << 1);"
...
Print #fOut, "            fire_script_success = 1;"
...
If actionsUsed (&HE1) Then
	...
	' &HE1
	If isSega Then
		Print #fOut, "                    case 0xe1: do_update_list_and_wait (); break;"
	Else
...
If actionsUsed (&HF4) Then
	...
	' &HF4 {xy, t}* &HFF
	If isSega Then
		Print #fOut, "                    case 0xf4: while (0xff != (sc_x = read_byte ())) { _x = sc_x >> 4; _y = sc_x & 15; _t = read_byte (); map_set (); do_update_list_and_wait (); clear_update_list (); } break;"
	Else
		If doubleWidth Then
...
If actionsUsed (&HE0) Then
	If isSega Then
		' SOUND n m
		' &HE0 n m
		Print #fOut, "                    case 0xe0: PSGSFXPlay (msc_sfx [read_vbyte ()], read_vbyte ()); break;"
	Else
		' SOUND n
		' &HE0 n
