...
; This file was generated Thu Nov 22 08:52:15 2018
;--------------------------------------------------------
	.module game
	.optsdcc -mz80
...
___str_5:
	...
	.db 0x00
;my/pres.h:81: void scr_level (void) {
;	---------------------------------
; Function scr_level
; ---------------------------------
_scr_level::
;my/pres.h:82: _x = 12; _y = 14; pr_str ("LEVEL 0"); SG_setTile (17+level);
	ld	hl,#__x + 0
	ld	(hl), #0x0C
	ld	hl,#__y + 0
	ld	(hl), #0x0E
	...
	call	_SG_setTile
	inc	sp
;my/pres.h:83: _x = 10; _y = 16; pr_str (levelnames [level]);
	ld	hl,#__x + 0
	ld	(hl), #0x0A
	ld	hl,#__y + 0
	ld	(hl), #0x10
...
___str_6:
	.ascii "LEVEL 0"
	.db 0x00
;my/pres.h:86: void credits (void) {
;	---------------------------------
; Function credits
; ---------------------------------
_credits::
;my/pres.h:87: cls ();
	call	_cls
;my/pres.h:89: _x = 0; _y = 0; if (ntsc) pr_str ("NTSC"); else pr_str ("PAL");
	ld	hl,#__x + 0
	ld	(hl), #0x00
	ld	hl,#__y + 0
	ld	(hl), #0x00
...
00102$:
	...
	pop	af
00103$:
;my/pres.h:91: rds16 = 0; rdy = 240;
	ld	hl,#0x0000
	ld	(_rds16),hl
	ld	hl,#_rdy + 0
	ld	(hl), #0xF0
;my/pres.h:92: _x = 0; _y = 18; 
	ld	hl,#__x + 0
	ld	(hl), #0x00
	ld	hl,#__y + 0
	ld	(hl), #0x12
;my/pres.h:93: pr_str ("     CHERIL PERILS CLASSIC%%         ORIGINAL GAME:%     @ 2011 THE MOJON TWINS%       REPROGRAMMED GAME:%     @ 2018 THE MOJON TWINS");
	ld	hl,#___str_12+0
	push	hl
	call	_pr_str
;my/pres.h:95: SG_displayOn ();
	ld	hl, #0x0140
	ex	(sp),hl
	call	_SG_VDPturnOnFeature
	pop	af
;my/pres.h:97: while (!(SG_getKeysStatus () & PAD_START) && rds16 < 300) {
00105$:
	call	_SG_getKeysStatus
	bit	4, l
	jr	NZ,00107$
...
;mainloop/flickscreen.h:48: py = 0;
	...
	ld	(_py),hl
	ret
;mainloop.h:6: void game_init (void) {
;	---------------------------------
; Function game_init
; ---------------------------------
_game_init::
;mainloop.h:8: win_level = game_over = 0;
	ld	hl,#_game_over + 0
	ld	(hl), #0x00
	ld	hl,#_win_level + 0
	ld	(hl), #0x00
...
;mainloop.h:16: cls ();
	call	_cls
;mainloop.h:18: draw_game_frame ();
	call	_draw_game_frame
;mainloop.h:21: hotspots_load ();
	call	_hotspots_load
;mainloop.h:24: memfill (lkact, 1, c_max_bolts);
	...
	pop	af
	inc	sp
;mainloop.h:27: if (!warp_to_level)	{
	ld	a,(#_warp_to_level + 0)
	or	a, a
	jr	NZ,00105$
;mainloop.h:28: n_pant = SCR_INI;
	ld	de,#_l_scr_ini+0
	ld	hl,(_level)
	ld	h,#0x00
	add	hl,de
	ld	a,(hl)
	ld	(#_n_pant + 0),a
;mainloop.h:29: px = (4 + (PLAYER_INI_X << 4)) << FIXBITS;
	ld	de,#_l_ini_x+0
	ld	hl,(_level)
	ld	h,#0x00
	add	hl,de
	...
	add	hl, hl
	ld	(_px),hl
;mainloop.h:30: py = (PLAYER_INI_Y << 4) << FIXBITS;
	ld	de,#_l_ini_y+0
	ld	hl,(_level)
	ld	h,#0x00
	add	hl,de
	...
	ld	(_py),hl
00105$:
;mainloop.h:32: player_to_pixels ();
	call	_player_to_pixels
;mainloop.h:33: player_init ();
	call	_player_init
;mainloop.h:36: enems_persistent_load ();
	call	_enems_persistent_load
;mainloop.h:40: enems_persistent_deaths_load ();
	call	_enems_persistent_deaths_load
;mainloop.h:52: pkeys = 0;
	ld	hl,#_pkeys + 0
	ld	(hl), #0x00
;mainloop.h:56: res_on = 0;
	ld	hl,#_res_on + 0
	ld	(hl), #0x00
;mainloop.h:57: res_disable = 0;
	ld	hl,#_res_disable + 0
	ld	(hl), #0x00
;mainloop.h:65: no_ct = 0;
	ld	hl,#_no_ct + 0
	ld	(hl), #0x00
;mainloop.h:104: half_life = 0;
	ld	hl,#_half_life + 0
	ld	(hl), #0x00
;mainloop.h:105: frame_counter = 0;
	ld	hl,#_frame_counter + 0
	ld	(hl), #0x00
;mainloop.h:106: olife = oammo = oobjs = okeys = 0xff;
	ld	hl,#_okeys + 0
	ld	(hl), #0xFF
	ld	hl,#_oobjs + 0
	ld	(hl), #0xFF
...
00121$:
	ld	(#_springs_on + 0),a
	ret
;mainloop.h:116: void prepare_scr (void) {
;	---------------------------------
; Function prepare_scr
; ---------------------------------
_prepare_scr::
;mainloop.h:117: SG_displayOff ();
	ld	hl,#0x0140
	push	hl
	call	_SG_VDPturnOffFeature
	pop	af
;mainloop.h:127: ft = 0;
	ld	iy,#_ft
	ld	0 (iy),#0x00
;mainloop.h:129: update_list [update_index] = 0xff;
...
;my/on_entering_screen.h:10: if (hrt == HOTSPOT_TYPE_RESONATOR) hrt = 0;
	...
	ld	(hl), #0x00
00104$:
;mainloop.h:229: gpit = 3; while (gpit --) en_spr_id [gpit] = en_s [gpit];
	ld	hl,#_gpit + 0
	ld	(hl), #0x03
00105$:
	ld	hl,#_gpit + 0
	...
	jr	00105$
00107$:
;mainloop.h:231: prx = px >> FIXBITS; pry = py >> FIXBITS;
	ld	hl,(_px)
	sra	h
	rr	l
	sra	h
	...
	ld	iy,#_pry
	ld	0 (iy),l
;mainloop.h:236: player_move ();
	call	_player_move
;mainloop.h:237: enems_move ();
	call	_enems_move
;mainloop.h:239: if (hrt) hotspots_paint ();
	ld	a,(#_hrt + 0)
	or	a, a
	jr	Z,00109$
	call	_hotspots_paint
00109$:
;mainloop.h:254: hud_update ();
	call	_hud_update
;mainloop.h:255: SG_copySpritestoSAT ();
	call	_SG_copySpritestoSAT
;mainloop.h:256: update_list [update_index] = 0xff;
	ld	a,(#_update_index + 0)
	add	a, #<(_update_list)
	ld	l, a
	ld	a, #0x00
	...
	ld	h, a
	ld	(hl),#0xFF
;mainloop.h:257: SG_doUpdateList ();
	call	_SG_doUpdateList
;mainloop.h:258: clear_update_list ();	
	call	_clear_update_list
;mainloop.h:259: SG_displayOn ();
	ld	hl,#0x0140
	push	hl
	call	_SG_VDPturnOnFeature
	pop	af
;mainloop.h:261: pad0 = 0;
	ld	hl,#_pad0 + 0
	ld	(hl), #0x00
	ret
;mainloop.h:264: void game_loop (void) {
;	---------------------------------
; Function game_loop
; ---------------------------------
_game_loop::
;mainloop.h:265: clear_update_list ();
	call	_clear_update_list
;mainloop.h:267: on_pant = 99; ft = 1; fade_delay = 1;
	ld	hl,#_on_pant + 0
	ld	(hl), #0x63
	ld	hl,#_ft + 0
	ld	(hl), #0x01
	ld	hl,#_fade_delay + 0
	ld	(hl), #0x01
;mainloop.h:271: SG_displayOn ();
	ld	hl,#0x0140
	push	hl
	call	_SG_VDPturnOnFeature
	pop	af
;mainloop.h:282: ntsc_frame = level_reset = warp_to_level = 0; 
	...
	ld	hl,#_ntsc_frame + 0
	ld	(hl), #0x00
;mainloop.h:283: ticker = 50;
	ld	hl,#_ticker + 0
	ld	(hl), #0x32
;mainloop.h:286: PSGPlay (l_music [level]);
	ld	iy,#_level
	ld	l,0 (iy)
	ld	h,#0x00
	add	hl, hl
	...
	call	_PSGPlay
	pop	af
;mainloop.h:291: while (1) {
00169$:
;mainloop.h:298: hud_update ();
	call	_hud_update
;mainloop.h:302: if (pkill) player_kill ();
	ld	a,(#_pkill + 0)
	or	a, a
	jr	Z,00102$
	call	_player_kill
00102$:
;mainloop.h:303: if (game_over || level_reset) break;			
	ld	a,(#_game_over + 0)
	or	a, a
	jp	NZ,00170$
	ld	a,(#_level_reset + 0)
	or	a, a
	jp	NZ,00170$
;mainloop.h:309: flick_override = 0;
	ld	hl,#_flick_override + 0
	ld	(hl), #0x00
;mainloop.h:312: flickscreen_do_horizontal ();
	call	_flickscreen_do_horizontal
;mainloop.h:313: flickscreen_do_vertical ();
	call	_flickscreen_do_vertical
;mainloop.h:319: if (on_pant != n_pant) {
	ld	a,(#_on_pant + 0)
	ld	iy,#_n_pant
	sub	a, 0 (iy)
	jr	Z,00109$
;mainloop.h:320: prepare_scr ();
	call	_prepare_scr
;mainloop.h:321: on_pant = n_pant;
	ld	a,(#_n_pant + 0)
	ld	(#_on_pant + 0),a
00109$:
;mainloop.h:340: SG_waitForVBlank ();
	call	_SG_waitForVBlank
;mainloop.h:341: SG_copySpritestoSAT ();
	call	_SG_copySpritestoSAT
;mainloop.h:342: update_list [update_index] = 0xff;
	ld	a,#<(_update_list)
	ld	hl,#_update_index
	add	a, (hl)
	ld	l, a
	...
	ld	h, a
	ld	(hl),#0xFF
;mainloop.h:343: SG_doUpdateList ();
	call	_SG_doUpdateList
;mainloop.h:344: clear_update_list ();
	call	_clear_update_list
;mainloop.h:348: pad_read ();
	call	_pad_read
;mainloop.h:349: a_button = (pad_this_frame & PAD_A);
	ld	a,(#_pad_this_frame + 0)
	and	a, #0x20
	ld	(#_a_button + 0),a
;mainloop.h:350: b_button = (pad_this_frame & PAD_B);
	ld	a,(#_pad_this_frame + 0)
	and	a, #0x10
	ld	(#_b_button + 0),a
;mainloop.h:354: ntsc_frame ++; if (ntsc_frame == 6) ntsc_frame = 0;
	ld	hl, #_ntsc_frame+0
	inc	(hl)
	ld	a,(#_ntsc_frame + 0)
	sub	a, #0x06
	...
	ld	(hl), #0x00
00111$:
;mainloop.h:356: if (paused == 0 && (ntsc == 0 || ntsc_frame)) {
	ld	a,(#_paused + 0)
	or	a, a
	jp	NZ,00161$
	ld	a,(#_ntsc + 0)
	...
	jp	Z,00161$
00160$:
;mainloop.h:357: SG_initSprites ();
	call	_SG_initSprites
;mainloop.h:360: if (ticker) -- ticker; else ticker = 50;
	ld	a,(#_ticker + 0)
	or	a, a
	jr	Z,00113$
	ld	iy,#_ticker
...
00113$:
	...
	ld	0 (iy),#0x32
00114$:
;mainloop.h:361: half_life ^= 1;
	ld	a,(#_half_life + 0)
	xor	a, #0x01
	ld	iy,#_half_life
	ld	0 (iy),a
;mainloop.h:362: ++ frame_counter;
...
;mainloop/hotspots.h:196: hact [n_pant] = 0;
	...
	ld	(de),a
00131$:
;mainloop.h:382: if (!warp_to_level) {
	ld	a,(#_warp_to_level + 0)
	or	a, a
	jr	NZ,00133$
;mainloop.h:383: player_move ();
	call	_player_move
00133$:
...
;mainloop/win_level_condition.h:25: break;
	jp	00170$
00141$:
;mainloop.h:420: cur_stp = SG_getStp (); 
	call	_SG_getStp
	ld	(_cur_stp),hl
;mainloop.h:421: if (!warp_to_level)	player_render ();
	ld	a,(#_warp_to_level + 0)
	or	a, a
	jr	NZ,00143$
	call	_player_render
00143$:
;mainloop.h:425: enems_move ();
	call	_enems_move
;mainloop.h:429: if (warp_to_level) {
	ld	a,(#_warp_to_level + 0)
	or	a, a
	jr	Z,00145$
;mainloop.h:430: update_cycle (); PSGStop (); break;
	call	_update_cycle
	call	_PSGStop
	jp	00170$
00145$:
...
;mainloop/resonators.h:19: PSGSFXPlay (SFX_SPRING, 1); 
	...
	inc	sp
00155$:
;mainloop.h:441: if (hrt) hotspots_paint ();
	ld	a,(#_hrt + 0)
	or	a, a
	jr	Z,00157$
	call	_hotspots_paint
...
;mainloop/pause.h:7: paused ^= 1;
	...
	jp	00169$
00170$:
;mainloop.h:485: PSGStop ();
	call	_PSGStop
;mainloop.h:486: PSGSFXStop ();
	call	_PSGSFXStop
;mainloop.h:487: SG_displayOff ();
	ld	hl,#0x0140
	push	hl
	call	_SG_VDPturnOffFeature
	pop	af
;mainloop.h:488: SG_initSprites ();
	call	_SG_initSprites
;mainloop.h:489: SG_copySpritestoSAT ();	
	jp  _SG_copySpritestoSAT
;game.c:127: void main(void) {
;	---------------------------------
; Function main
; ---------------------------------
_main::
;game.c:128: SG_displayOff ();
	ld	hl,#0x0140
	push	hl
	call	_SG_VDPturnOffFeature
;game.c:129: SG_setSpriteMode (SG_SPRITEMODE_LARGE);
	ld	h,#0x01
	ex	(sp),hl
	inc	sp
	call	_SG_setSpriteMode
	inc	sp
;game.c:130: SG_setUpdateList (update_list);
	ld	hl,#_update_list
	push	hl
	call	_SG_setUpdateList
	pop	af
;game.c:131: first_game = 1;
	ld	hl,#_first_game + 0
	ld	(hl), #0x01
;game.c:136: ntsc = 1;
	ld	hl,#_ntsc + 0
	ld	(hl), #0x01
;game.c:140: unpack_bg_patterns (tsfont_patterns_c, tsfont_colours_c, 0, 7);
...
;game.c:142: credits ();
	call	_credits
;game.c:145: aPLib_depack_VRAM (SGT_BASE, ss_fixed_patterns_c);
	...
	pop	af
	pop	af
;game.c:147: mode_no_resonators = 0;
	ld	hl,#_mode_no_resonators + 0
	ld	(hl), #0x00
;game.c:148: while (1) {	
00110$:
...
;game.c:151: level = 0;
	ld	hl,#_level + 0
	ld	(hl), #0x00
;game.c:152: plife = PLAYER_LIFE;
	ld	hl,#_plife + 0
	ld	(hl), #0x05
;game.c:156: while (1) {
00107$:
;game.c:157: pres (scr_level, 0);
	ld	hl,#0x0000
	push	hl
	ld	hl,#_scr_level
	push	hl
...
;game.c:165: level ++;
	ld	hl, #_level+0
	inc	(hl)
;game.c:166: if (level == MAX_LEVELS) {
	ld	a,(#_level + 0)
	sub	a, #0x03
	jr	NZ,00107$
