...
; This file was generated Tue Sep 10 14:20:17 2019
;--------------------------------------------------------
	.module player
	.optsdcc -mz80
...
;./engine/player.c:227: pnotsafe = 0;
	ld	hl,#_pnotsafe + 0
	ld	(hl), #0x00
;./engine/player.c:237: cx1 = prx >> 4; cx2 = (prx + 7) >> 4;
	ld	a,(#_prx + 0)
	rlca
	rlca
	rlca
	...
	ld	iy,#_cx2
	ld	0 (iy),l
;./engine/player.c:238: cy1 = cy2 = (pry + 15) >> 4;
	ld	iy,#_pry
	ld	l,0 (iy)
	ld	h,#0x00
	ld	bc,#0x000F
	...
	ld	(#_cy2 + 0),a
	ld	(#_cy1 + 0),a
;./engine/player.c:239: cm_two_points ();
	call	_cm_two_points
;./engine/player.c:248: rda = pfloating;
	ld	a,(#_pfloating + 0)
	ld	(#_rda + 0),a
;./engine/player.c:249: pfloating = (at1 == 64 || at2 == 64);
	ld	a,(#_at1 + 0)
	sub	a, #0x40
	jr	Z,00212$
	ld	a,(#_at2 + 0)
...
00213$:
	ld	(#_pfloating + 0),a
;./engine/player.c:250: if (rda != pfloating) PSGSFXPlay (SFX_FLOAT, 2);
	ld	a,(#_rda + 0)
	ld	iy,#_pfloating
	sub	a, 0 (iy)
	jr	Z,00106$
...
;./engine/player.c:268: ppossee = 0;
	ld	hl,#_ppossee + 0
	ld	(hl), #0x00
;./engine/player.c:334: if (pfloating) {
	ld	a,(#_pfloating + 0)
	or	a, a
	jr	Z,00116$
;./engine/player.c:335: pvy -= PLAYER_AY_FLOAT; 
	ld	hl,#_pvy
	ld	a,(hl)
	add	a,#0xF0
	ld	(hl),a
	...
	adc	a,#0xFF
	ld	(hl),a
;./engine/player.c:336: if (pvy < -PLAYER_VY_FLOAT_MAX) pvy = -PLAYER_VY_FLOAT_MAX;
	ld	a,(#_pvy + 1)
	xor	a, #0x80
	sub	a, #0x7F
	jr	NC,00117$
	...
	jr	00117$
00116$:
;./engine/player.c:340: if (vertical_engine_type != ENGINE_TYPE_SWIM) {
	ld	a,(#_vertical_engine_type + 0)
	sub	a, #0x02
	jr	Z,00117$
;./engine/player.c:351: if (!pj) {
	ld	a,(#_pj + 0)
	or	a, a
	jr	NZ,00117$
;./engine/player.c:352: pvy += PLAYER_G;
	ld	hl,#___player_g + 0
	ld	d, (hl)
	ld	e,#0x00
	ld	hl,#_pvy
	...
	adc	a, e
	ld	(hl),a
;./engine/player.c:353: if (pvy > PLAYER_VY_FALLING_MAX) pvy = PLAYER_VY_FALLING_MAX; 
...
00449$:
	...
	ld	(_pvy),de
00117$:
;./engine/player.c:360: if (pgotten) pvy = 0;			
	ld	a,(#_pgotten + 0)
	or	a, a
	jr	Z,00119$
	ld	hl,#0x0000
	ld	(_pvy),hl
00119$:
;./engine/player.c:366: if (vertical_engine_type == ENGINE_TYPE_JET_PAC) {
	ld	a,(#_vertical_engine_type + 0)
	dec	a
	jr	NZ,00127$
;./engine/player.c:367: if (pad0 & PAD_A) {
	ld	hl,#_pad0+0
	bit	5, (hl)
	jr	Z,00127$
;./engine/player.c:368: pvy -= PLAYER_AY_JETPAC;
	...
	sbc	a, e
	ld	(hl),a
;./engine/player.c:370: if (!pfloating)
	ld	a,(#_pfloating + 0)
	or	a, a
	jr	NZ,00127$
;./engine/player.c:372: if (pvy < -PLAYER_VY_JETPAC_MAX) pvy = -PLAYER_VY_JETPAC_MAX;
...
;./engine/player.c:408: cx2 = (prx + 7) >> 4;
	...
	ld	iy,#_cx2
	ld	0 (iy),l
;./engine/player.c:413: rds16 = pvy + pgtmy;
	ld	hl,#_pgtmy
	push	de
	ld	iy,#_rds16
	push	iy
...
;./engine/player.c:238: cy1 = cy2 = (pry + 15) >> 4;
	...
	ld	e,0 (iy)
	ld	d,#0x00
;./engine/player.c:414: if (rds16 < 0)
	ld	a,(#_rds16 + 1)
	bit	7,a
	jp	Z,00153$
...
;./engine/player.c:418: cm_two_points ();
	call	_cm_two_points
;./engine/player.c:422: if ((at1 & 8) || (at2 & 8)) 
	ld	iy,#_at1
	bit	3, 0 (iy)
	jr	NZ,00133$
	ld	iy,#_at2
...
;./engine/player.c:435: hitv = 1;
	...
	jp	00154$
00153$:
;./engine/player.c:446: } else if (rds16 > 0)
	xor	a, a
	ld	iy,#_rds16
	cp	a, 0 (iy)
	ld	iy,#_rds16
...
;./engine/player.c:450: cm_two_points (); 
	call	_cm_two_points
;./engine/player.c:455: pry < ((cy1 - 1) << 4) + 4 && 
	ld	iy,#_cy1
	ld	l,0 (iy)
	ld	h,#0x00
	dec	hl
...
00461$:
	jp	P,00146$
;./engine/player.c:457: (at1 & 12) || (at2 & 12)
	ld	a,(#_at1 + 0)
	and	a, #0x0C
	jr	NZ,00145$
	ld	a,(#_at2 + 0)
...
00137$:
	...
	jr	00154$
00146$:
;./engine/player.c:505: cy2 = pry + 15 - PLAYER_SPIKES_BOTTOM_ALLOW;
	ld	hl,#_cy2
	ld	a,(#_pry + 0)
	add	a, #0x0D
	ld	(hl),a
;./engine/player.c:506: cm_two_points ();
	call	_cm_two_points
...
;./engine/player.c:698: if (pad0 & PAD_LEFT) {
	...
	or	a, a
	jr	Z,00170$
;./engine/player.c:702: pfacing = CELL_FACING_LEFT;		
	ld	iy,#_pfacing
	ld	0 (iy),#0x05
;./engine/player.c:705: if (pvx > -PLAYER_VX_MAX) {
	xor	a, a
...
;./engine/player.c:715: if (pad0 & PAD_RIGHT) {
	...
	or	a, a
	jr	Z,00174$
;./engine/player.c:719: pfacing = CELL_FACING_RIGHT;
	ld	iy,#_pfacing
	ld	0 (iy),#0x00
...
;./engine/player.c:733: px += pvx;
	...
	ld	(de),a
	pop	de
;./engine/player.c:735: if (pgotten) px += pgtmx;
	ld	a,(#_pgotten + 0)
	or	a, a
	jr	Z,00176$
	ld	hl,#_pgtmx
...
;./engine/player.c:779: cm_two_points ();
	call	_cm_two_points
;./engine/player.c:783: if ((at1 & 8) || (at2 & 8)) 
	ld	hl,#_at1+0
	bit	3, (hl)
	jr	NZ,00186$
	ld	hl,#_at2+0
...
;./engine/player.c:786: pvx = 0; prx = rda; px = prx << FIXBITS; pfiring = 1;
	...
	jr	00188$
00187$:
;./engine/player.c:795: cy2 = pry + 15 - PLAYER_SPIKES_BOTTOM_ALLOW;
	ld	hl,#_cy2
	ld	a,(#_pry + 0)
	add	a, #0x0D
	ld	(hl),a
;./engine/player.c:796: cm_two_points ();
	call	_cm_two_points
...
00222$:
	...
	ld	(_px),hl
00198$:
;./engine/player.c:847: cx1 = cx2 = (prx + 4) >> 4;
	ld	hl,#_prx + 0
	ld	e, (hl)
	ld	d,#0x00
	inc	de
	...
	ld	hl,#_cx1 + 0
	ld	(hl), e
;./engine/player.c:848: cy1 = (pry - PLAYER_COLLISION_VSTRETCH_BG) >> 4; cy2 = (pry + 15) >> 4;
	ld	hl,#_pry + 0
	ld	e, (hl)
	ld	d,#0x00
	ld	a,e
	...
	ld	iy,#_cy2
	ld	0 (iy),l
;./engine/player.c:849: cm_two_points ();
	call	_cm_two_points
;./engine/player.c:850: if ((at1 & 1) || (at2 & 1)) phit = 1;
	ld	hl,#_at1+0
	bit	0, (hl)
	jr	NZ,00199$
	ld	hl,#_at2+0
